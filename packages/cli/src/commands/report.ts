        ...
        const cmd =
          process.platform === 'darwin'
            ...
            : process.platform === 'win32'
              ? 'start'
              : 'xdg-open';
        execFile(cmd, [htmlPath], (err) => {
          if (err) logger.warn('Failed to open report', { error: err.message });
        });
      }
    });
}
...
function generateMarkdownReport(runResult: RunResult): string {
  ...
  if (failed.length > 0) {
    ...
    for (const t of failed) {
      ...
      const file = t.file.replace(/\|/g, '\\|').replace(/\n/g, ' ');
      lines.push(`| ${title} | ${file} | ${formatDuration(t.duration)} |`);
    }
    lines.push('');
  }
  ...
  if (flaky.length > 0) {
    ...
    for (const t of flaky) {
      const title = t.title.replace(/\|/g, '\\|').replace(/\n/g, ' ');
      const file = t.file.replace(/\|/g, '\\|').replace(/\n/g, ' ');
      lines.push(`| ${title} | ${file} | ${t.retries} |`);
    }
    lines.push('');
  }
  ...
  lines.push(
    '---',
    '*Generated by [sorry-currents](https://github.com/shand-j/sorry-currents)*',
    '',
  );
  return lines.join('\n');
}

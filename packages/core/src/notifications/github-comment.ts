...
export function buildGitHubCommentBody(options: GitHubCommentOptions): GitHubCommentPayload {
  ...
  const statusIcon = runResult.status === 'passed' ? '✅' : '❌';
  const lines: string[] = [
    COMMENT_MARKER,
  ...
  if (failed.length > 0) {
    ...
    for (const t of failed) {
      ...
      const safeFile = t.file.replace(/`/g, '\\`');
      const safeTitle = t.title.replace(/`/g, '\\`');
      lines.push(`| \`${safeFile} > ${safeTitle}\` | ${escaped} | ${formatDuration(t.duration)} |`);
    }
    lines.push('');
  }
  ...
  if (flaky.length > 0) {
    lines.push('### ⚠️ Flaky Tests (passed on retry)', '');
  ...
  const footer: string[] = [];
  if (reportUrl) {
    footer.push(`📊 [Full Report](${reportUrl})`);
  }
  footer.push('*Generated by [sorry-currents](https://github.com/shand-j/sorry-currents)*');
